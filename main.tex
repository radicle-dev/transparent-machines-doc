...
\begin{abstract}
  This document presents a vision for a possible future of Radicle.  The main
  goal is to transform Radicle into a technology that powers an ecosystem of
  \emph{transparent state machines}. This is a network of replicated state
  machines running the Radicle interpreter which are monitored by their peers,
  to ensure they abide by the Radicle semantics.
\end{abstract}
...
\begin{itemize}
\item A deterministic programming language that is designed in order to be able
  to specify state machines whose semantics may evolve over time.
\item A daemon which allows hosting and reading such machines on IPFS, using:
  \begin{itemize}
  \item The IPFS DAG to store the list of blocks containing the Radicle
    expressions which determine the machine,
  \item IPNS as a mutable reference to the head of the list.
  \item IPFS Pubsub for communication of suggested inputs from peers to the
    owner of a machine.
  \end{itemize}
\end{itemize}
...
\begin{itemize}
  \item \emph{Unweildy reprogrammability.} The current design of the language
    has not proved itself to allow for easy reprogramability of machines.
  \item \emph{No guarantee of Radicle semantics.} The machine owner has complete
    ...
    whitepaper \cite{radicle}. This voids a lot of the supposed benefits of
...
\begin{itemize}
  \item A P2P system for \emph{monitoring} Radicle machines which, while giving
    no guarantee that owners will follow Radicle semantics, make the detection
    of misbehaviour evident at the protocol level. Peers simply ignore
  ...
  \item A simpler mechanism for restricting inputs, based on an updatable
    validation function, rather than eval-redefinition, along with a non-hyper
    static environment to allow for better reprogrammability.
  \item Finally the potential censorship problem is partially tackled by making
    ...
    stage.
  \item
    To mitigate the owner availability burden, we propose that potential inputs
    are kept track of by peers in a CRDT, similar to an OrbitDB log
    \cite{orbitdb}.
\end{itemize}
...
increase the faith of their citizens (\eg \cite{ukraine-transparency}), so there
...
\begin{itemize}
  \item Protocol re-use: Having many systems use the same underlying protocol
    makes that protocol safer, more resources can be spent in auditing its code,
    fixing vulnerabilities, etc.
  \item Radicle code re-use: domain-logic specifying rules is likely to be
    similar across domains and organisations. For example many domains will
    require processes for holding votes, electing leaders, managing user
    permissions, etc. By creating a set of ready-to-use packages each new
    organisation may launch a tailored transparent machine with minimal effort.
  \item Larger, homogeneous network of monitors: Once a machine is launched it
    immediately benefits from a large network on monitors. Different domains can
    now share monitor nodes, because each monitor just needs to understand the
...
\begin{itemize}
  ...
  \item \emph{Transparent Certificate} in general, including:
    \begin{itemize}
      ...
      \item A paper:
        \href{https://arxiv.org/pdf/1806.08817.pdf}{Aggregation-Based Gossip for
...
might use an IPLD~\cite{ipld}, a DAT hypercore feed~\cite{dat} or a Trillian log~\cite{trillian}. The description below
will try to remain agnostic to such decisions.
...
\begin{itemize}
\item
  \emph{Radicle-lang} refers to the Radicle programming language: a grammar for
  valid expressions and an deterministic \emph{interpreter} which maintains
  state (or produces an error) when ran over a sequence of expressions.
\item
  ...
  interpreter.
\item
  A \emph{secure ledger} is a list of records, called \emph{blocks}, which are
  linked using cryptography. Each block contains a cryptographic hash of the
  previous block, and transaction data. In our case the transaction data will be
  a list of Radicle expressions. Each block is signed by the same private key,
  whose corresponding public $p$ key identifies the ledger as a whole. We will
  simply say \emph{ledger} to mean secure ledger in this document, and
  \emph{ledger for $p$} when we wish to specify the corresponding public key.
\item
...
\item Two snapshots of a secure ledger are \emph{consistent} if one is a prefix
  of the other.
\end{itemize}
...
log quite seriously, and when unmonitoring a machine should either persist the
last ledger it has seen, or persist the machine's ID so as to make sure to never
...
\hypertarget{radicle-lang-simplificationimprovements}{%
\section{Radicle-lang
simplification/improvements}\label{radicle-lang-simplificationimprovements}}

\subsection{Simpler input
restriction}\label{simpler-input-restriction}
...
used for all inputs to the machine. Whatever this function returns is what's
finally evaluated. Furthermore, the Radicle executable can be configured to
bypass the root-validator using a simple flag, for when one wants to experiment
with the machine at the REPL or craft specialised queries. Daemons which are
owner-mode for a machine should obviously never bypass the root validator when
accepting new inputs, but the \texttt{query} endpoint on a local daemon would
just execute expressions normally, without calling \texttt{tx}.
...
Note that querying has now been pushed out of the machine code altogether; if one wants to query the machine for the current value one can just use the expression
...
A validator is used to discard invalid comments.
...
list of comments as of that checkpoint.

%% \section{P2P reference types}\label{radicle-crdts}

%% At the moment a machine must be available to accept inputs at all times. This
%% means that participants of a machine can't submit new items (issues,
%% merge-requests, comments, etc.) until the machine comes back online. 

%% At the moment machines are single-writer, and as a consequence the owner must be
%% available to accept new inputs at all times. This isn't really a restriction for
%% the sequence of expressions which define the semantics of the machine, since
%% such inputs are quite rare. But for bread-and-butter inputs adding user-data
%% this can be a burden.

%% At the moment such (domain-level) state of a machine is mostly held in
%% \texttt{ref}s, and the contents of a ref is determined by the sequence
%% of \texttt{write-ref}s that have targeted that ref. Each of these
%% commands must be in an input expression that is included in a block by a
%% machine.

%% Another possibility might be for Radicle to contain special
%% (multiwriter) \emph{P2P} mutable datatypes alongside refs, which are
%% used to contain specific parts of the state of a machine.

%% One such example might be a \emph{multi-writer mutable dict}. This is a
%% dict from string keys to arbitrary values with the following properties:
%% - The dict is initiated with a validator \texttt{v} and a
%% conflict-resolver \texttt{r}, both are functions.

%% TBC

%% So Julian is investigating DAT. I have been thinking some about ``adding
%% various p2p datatypes'' to radicle machines.

%% A basic example is adding a reference to another machine. This is
%% actually what we already do in the project machine in some sense.

%% But one of the queries for state on a machine might also reference the
%% state of another machine. The level of ``consistency'' here is that, any
%% 2 snapshots of the machines, when combined, provide a view into this
%% combined state.

%% More basically you might add a reference to an append only log, but that
%% maybe someone else than the machine owner has the public key. For
%% example each new ``admin'' to a machine might get their own log, and
%% then when the state of the machine is queried it gets all these logs
%% too, to get the inputs of all the admins.

%% The most extreme thing I might want to add is a ``multiwriter DAT dict
%% ref'', this would be a reference type that has roughly the API of a a
%% dict under a ref, however it has other operations like ``add writer'',
%% etc. Also when you \texttt{get} a key you get back a list of possible
%% values (because the consensus mechanism might have conflicts), so at
%% creation of the multiwriter-dict you would also provide a lambda which
%% is used for resolving these conflicts.

%% This is in fact not really anything new. In some sense I am just saying:
%% because we have the \texttt{query} fn in radicle, it's possible to craft
%% a radicle expression that queries more than one machine, gets some data
%% back, and puts it together. For example one could do this to get a
%% combined view of the issues and patches of a project and maybe highlight
%% the cross-references. Should we encourage this sort of thing?

%% It seems likely that if we create a ``identity'' machine, where people
%% claim usernames with public keys, a lot of other machines might craft
%% queries which reference it. (for reference, the \texttt{get} key API:
%% https://github.com/mafintosh/hyperdb\#dbgetkey-callback)

%% semi-related:

%% The above has made me realise that the fundamental queries really
%% \emph{should} be ``on-chain''. Otherwise, granted there is all the state
%% and transactional functions on-chain, but people can still disagree on
%% stuff. For example the patches machine. It was stated that one can
%% derive from the state what the state of the repo is (give some other
%% assumptions like no off-machine commits). But this derivation fn is
%% non-trivial, people could

%% still argue over what is ``the state of the repo as of this CID''.
